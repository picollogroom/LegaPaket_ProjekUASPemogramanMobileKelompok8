...
class KelolaLaporan_admin : AppCompatActivity() {
    ...
    private fun generateLaporan() {
        lifecycleScope.launch {
            var hasil: List<Paket>
            var statusFilter = "Semua Status"
            var kotaFilter = "Semua Kota"
            var tipeData = "Semua Pengiriman"

            // ==== BAGIAN 1: Ambil & filter data, terus TAMPILIN. Ini harus selalu jalan. ====
            try {
                ...
                statusFilter = spinnerStatus.selectedItem?.toString() ?: "Semua Status"
                ...
                kotaFilter = spinnerKota.selectedItem?.toString() ?: "Semua Kota"
                if (kotaFilter != "Semua Kota") {
                    hasil = hasil.filter { it.kotaTujuan == kotaFilter }
                }

                tipeData = spinnerTipeData.selectedItem?.toString() ?: "Semua Pengiriman"
                ...
                Toast.makeText(this@KelolaLaporan_admin, "Gagal memuat data: ${e.message}", Toast.LENGTH_LONG).show()
                return@launch
            }

            // ==== BAGIAN 2: SIMPAN ke riwayat. Kalau ini gagal, tampilan di atas TETAP muncul. ====
            try {
                ...
                val laporan = Laporan(
                    ...
                    dibuatOleh = SessionManager.getNama(this@KelolaLaporan_admin),
                    items = hasil.map {
                        LaporanItem(
                            noResi = it.noResi,
                ...
                Toast.makeText(this@KelolaLaporan_admin, "Laporan disimpan ke riwayat", Toast.LENGTH_SHORT).show()
            } catch (e: Exception) {
                Toast.makeText(this@KelolaLaporan_admin, "Laporan ditampilkan, tapi gagal disimpan ke riwayat: ${e.message}", Toast.LENGTH_LONG).show()
            }
        }
    }
